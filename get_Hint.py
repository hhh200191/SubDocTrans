...
from helpers.ollama_df_helpers import  documents2Dataframe,  split_by_lines
from helpers.gpt_prompts import  df2Hint
...
def process_hint(pages: list, src_lang: str, tgt_lang: str, language_dict: dict, model: str) -> tuple:
    """提取三元组并生成映射字典"""
    df = documents2Dataframe(pages)

    Summary = df2Hint(df, model=model, src_lang=src_lang, tgt_lang=tgt_lang, dic=language_dict)

    return Summary


def save_data(hint: list, output_dir: Path, file_name: str, expected_length: int) -> None:
    """保存hint JSON 文件到指定目录"""
    ...
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(hint, f, ensure_ascii=False, indent=4)
    print(f"JSON 保存完成，路径: {json_path}")

    # 检查长度并保存错误日志
    if len(hint) != expected_length:
        error_log_path = output_dir / "error_log.txt"
        error_message = f"hint长度 {len(hint)} 不匹配预期 {expected_length}"
...
def main():
    ...
    for txt_file in txt_files:
        ...
        while retries <= MAX_RETRIES:
            hint = process_hint(pages, SRC_LANG, TGT_LANG, LANGUAGE_DICT, MODEL)
            
            if len(hint) == expected_length:
                print(f"主题长度 {len(hint)} 匹配预期 {expected_length}")
                break
            else:
                print(f"主题长度 {len(hint)} 不匹配预期 {expected_length}，重试...")
                retries += 1

        print(f"Hint 提取完成，耗时: {time.time() - step_start:.2f} 秒")


        # 保存数据
        step_start = time.time()
        save_data(hint, output_dir, file_name, expected_length)
