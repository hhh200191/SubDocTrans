...
from helpers.ollama_df_helpers import df2Graph, documents2Dataframe, graph2Df, split_by_lines, df2Summary

from helpers.gpt_prompts import df2Topic
...
def process_topic(pages: list, src_lang: str, tgt_lang: str, language_dict: dict, model: str) -> tuple:
    """提取三元组并生成映射字典"""
    df = documents2Dataframe(pages)

    topic = df2Topic(df, model=model, src_lang=src_lang, tgt_lang=tgt_lang, dic=language_dict)

    return topic


def save_data(topic: list, output_dir: Path, file_name: str, expected_length: int) -> None:
    """保存 JSON 文件到指定目录，并在长度不匹配时保存错误日志"""
    ...
    # 检查长度并保存错误日志
    if len(topic) != expected_length:
        error_log_path = output_dir / "error_log.txt"
        error_message = f"主题长度 {len(topic)} 不匹配预期 {expected_length}"
...
def main():
    ...
    for txt_file in txt_files:
        ...
        while retries <= MAX_RETRIES:
            topic = process_topic(pages, SRC_LANG, TGT_LANG, LANGUAGE_DICT, MODEL)
            
            if len(topic) == expected_length:
                print(f"主题长度 {len(topic)} 匹配预期 {expected_length}")
                break
            else:
                print(f"主题长度 {len(topic)} 不匹配预期 {expected_length}，重试...")
                retries += 1
        ...
        # 保存数据
        step_start = time.time()
        save_data(topic, output_dir, file_name, expected_length)
